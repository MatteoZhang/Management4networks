...
def global_service_times(server, dict, current_time, current_capacity):
    for i in (dict[server]["current_requests"].keys()):
        ...
        dict[server]["current_requests"][i][2] = dict[server]["current_requests"][i][2] - \
                                                 dict[server]["current_requests"][i][1] * (
                                                             current_time - dict[server]["current_requests"][i][3])
        ...
        dict[server]["current_requests"][i][0] = dict[server]["current_requests"][i][2] / \
                                                 dict[server]["current_requests"][i][1]
